...
try:
    ...
    from colorama import Fore, Style as ColoramaStyle, init, Back
    from pystyle import Add, Center, Anime, Colors, Colorate, Write, System
except ImportError:
    os.system("pip install requests rich colorama pystyle beautifulsoup4 selenium mechanize webdriver_manager")
    print("Đã cài đặt thư viện xong. Vui lòng chạy lại tool!")
    sys.exit()
...
# ==============================================================================
#  PHẦN LOGIN SYSTEM (VƯỢT LINK LẤY KEY)
# ==============================================================================

LINK_LAY_KEY = "https://link4m.com/hyttdb2"
SECRET_CODE = "080711huydzngau"

# Màu sắc cơ bản cho phần Login
den = '\x1b[1;90m'
luc = '\x1b[1;32m'
trang = '\x1b[1;37m'
red = '\x1b[1;31m'
...
def show_banner_login():
    clear_screen_login()
    print(Colorate.Horizontal(Colors.green_to_white, Center.XCenter(banners_login)))
...
def login_system():
    ...
    while True:
        try:
            # Sử dụng input thường để tránh lỗi buffer trên Termux
            user_key = input(f"{luc}root@huycon:~# {trang}Nhập Key: {vang}").strip()

            if user_key == true_key:
                print(f"\n{luc} [✔] Key Chính Xác! Đang vào tool...{reset}")
                sys.stdout.flush() # Xả bộ nhớ đệm để tránh crash
                time.sleep(1)
                break
            elif user_key == "ADMINHUYCON":
...
# ==============================================================================
#  HÀM VISUAL: TRẠM KIỂM SOÁT (DÙNG CHUNG)
# ==============================================================================

def tram_kiem_soat_visual(tool_name):
    """
    Hiệu ứng Trạm Kiểm Soát (Checkpoint)
    """
    os.system('cls' if os.name == 'nt' else 'clear')
    console.print(Panel(f"[bold yellow]🔒 TRẠM KIỂM SOÁT AN NINH - {tool_name}[/]", style="bold red"))
    
    steps = [
        "Đang kết nối máy chủ vệ tinh...",
        "Đang xác minh danh tính Master Huycon...",
        "Kiểm tra dữ liệu đầu vào...",
        "Bẻ khóa tầng bảo mật lớp 1...",
        "Xác thực thành công! Đang chuyển hướng..."
    ]
    
    with console.status(f"[bold green]Đang kiểm tra dữ liệu cho {tool_name}...[/]", spinner="dots"):
        for step in steps:
            time.sleep(0.5) # Giảm độ trễ chút cho mượt
            console.print(f"[cyan]➤ {step}[/]")
    
    console.print(f"\n[bold green]✅ TRẠM KIỂM SOÁT HOÀN TẤT! CHÀO MỪNG ĐẾN {tool_name}[/]")
    time.sleep(1.0)

# ==============================================================================
#  PHẦN 1: TOOL VIP VTH (VUA THOÁT HIỂM - ESCAPE MASTER)
# ==============================================================================

CONFIG_FILE_VTH = "config_huycon_omega_vn.json"
